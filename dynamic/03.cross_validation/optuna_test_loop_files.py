...
OUTPUT_DIR = os.path.join("output\output_optuna")
...
def load_processed_log():
    ...
    if os.path.exists(CHECKPOINT_FILE):
        try:
            with open(CHECKPOINT_FILE, "r") as f:
                data = f.read().strip()  # อ่านข้อมูลทั้งหมดและตัดช่องว่าง
                if not data:  # ถ้าไฟล์ว่างเปล่า
                    return set()
                return set(json.loads(data))  # Convert JSON list to a set for quick lookup
        except json.JSONDecodeError:
            logging.warning(f"Corrupted JSON detected in {CHECKPOINT_FILE}. Resetting file.")
            return set()  # คืนค่าเป็นเซตว่างและดำเนินการต่อ
    return set()
...
if __name__ == '__main__':
    ...
    datasets = joblib.load((f'{INPUT_DIR}/pulsar_resamples_list_1_chunks18.pkl'))
