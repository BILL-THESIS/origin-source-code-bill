...
if __name__ == "__main__":
    ...
    # Check the data frame original data to set tag labels form divide quantile 25% and 75%
    ...
    # Evaluate the model with all factors
    ...
    results_evaluated_all.append({
        ...
        'auc': roc_auc_score(y_test, y_probs[:, 1], multi_class='ovr', average='macro')
    })

    results_evaluated_all = pd.DataFrame(results_evaluated_all)

    # Evaluate the model one factor at a time (per feature)
    ...
    for column in X.columns:
        ...
        y_probs_column = model.predict_proba(X_test_column)
        ...
        results_metrics_list.append({
            ...
            'auc': roc_auc_score(y_test, y_probs_column[:, 1], multi_class='ovr', average='macro')
        })
