...
def load_data(input_filepath="../../output/pulsar_compare.pkl",
              group_filepath="../../output/pulsar_correlation_group.pkl"):
...
def evaluate_features_with_importance(X, y, model, cv, scoring):
    """ประเมินฟีเจอร์และคำนวณ feature importance"""
...
if __name__ == "__main__":
    ...
    # ใช้ข้อมูลทั้งหมดเพื่อตั้งค่าพารามิเตอร์ที่ดีที่สุด
    X_all = data.drop(columns=['time_category', 'total_time', 'total_hours']).fillna(0)
    ...
    with ProcessPoolExecutor(max_workers=8) as executor:
        futures = [
            executor.submit(process_feature_group, feature_set, data, y, best_model, cv, scoring)
    ...
    # สร้าง DataFrame สำหรับค่าพารามิเตอร์ที่ดีที่สุด
    best_params_df = pd.DataFrame([best_params])

    # เพิ่ม timestamp เพื่อไม่ให้ชื่อไฟล์ซ้ำ
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

    results_filename = f"../../output/pulsar_rdf_quantile_all_{timestamp}.pkl"
    importance_filename = f"../../output/pulsar_feature_importances_{timestamp}.pkl"
    best_params_filename = f"../../output/pulsar_best_params_{timestamp}.pkl"

    logging.info("Evaluation complete. Saving results...")
    final_results_df.to_pickle(results_filename)
    feature_importances_df.to_pickle(importance_filename)
    best_params_df.to_pickle(best_params_filename)

    logging.info(f"Results saved: {results_filename}")
    logging.info(f"Feature importances saved: {importance_filename}")
    logging.info(f"Best parameters saved: {best_params_filename}")