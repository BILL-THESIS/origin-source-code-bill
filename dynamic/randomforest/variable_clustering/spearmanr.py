...
if __name__ == "__main__":
    ...
    corr_matrix, p_value = spearmanr(data_cleaned)
    corr_matrix = pd.DataFrame(corr_matrix, index=data_cleaned.columns, columns=data_cleaned.columns)
    p_value = pd.DataFrame(p_value, index=data_cleaned.columns, columns=data_cleaned.columns)
    corr_matrix_arr = np.nan_to_num(corr_matrix)
    p_value_arr = np.nan_to_num(p_value)
    ...
    linkage_matrix = linkage(df_distance_matrix,  metric='mahalanobis')
    ...
    # Print distances to each clustering
    distances = linkage_matrix[:, 2]

    # Agglomerative clustering
    cluster_agg = AgglomerativeClustering(n_clusters=3, affinity='precomputed', linkage='ward')
