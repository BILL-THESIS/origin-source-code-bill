...
if __name__ == "__main__":
    ...
    file_feature_importances = '../../output/seatunnel_feature_importances.pkl'
    ...
    data_rank_group_f1 = grop_80_percen['features'].apply(lambda s: [group_join.set_index('features').loc[x, 'rank_f1'] for x in s])
    data_rank_group_f1 = pd.concat([grop_80_percen['features'],grop_80_percen['test_f1'] ,data_rank_group_f1], axis=1)
    df_split_f1 = pd.DataFrame(data_rank_group_f1.iloc[:, 2].tolist(), columns=["G1", "G2", "G3", "G4"],
                               index=data_rank_group_f1.index)
    data_rank_group_f1 = pd.concat([data_rank_group_f1, df_split_f1], axis=1)

    data_rank_group_f1['sum'] = data_rank_group_f1[['G1', 'G2', 'G3', 'G4']].sum(axis=1)
