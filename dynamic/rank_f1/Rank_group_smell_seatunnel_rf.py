...
if __name__ == '__main__':
    ...
    # Load the data
    pulsar = pd.read_pickle('../output/seatunnal_cut_time.pkl')
    pulsar_group = pd.read_pickle("../01.variable_clustering/output_variable/seatunnel_correlation_main_group.pkl")
    pulsar_optuna = joblib.load("../randomforest/output_randomforest/seatunnel_optuna_result_each_smell_rdf.pkl")
    pulsar_optuna_all_case = joblib.load("../randomforest/output_randomforest/seatunnel_optuna_result_combinations_new_rf.pkl")
    ...
    # df_rank_new.to_pickle("output_rank/seatunnal_optuna_result_rank_rf.pkl")

    df_rank_new = df_rank_new.sort_values(by=['sum'], ascending=True).head(10)

    df_rank_new.describe()
