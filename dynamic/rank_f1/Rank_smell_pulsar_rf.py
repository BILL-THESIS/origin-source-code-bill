...
if __name__ == '__main__':
    ...
    pulsar_optuna = joblib.load("../randomforest/output_randomforest/pulsar_optuna_result_each_smell_rdf.pkl")
    pulsar_optuna_all_case = joblib.load("../randomforest/output_randomforest/pulsar_optuna_result_rdf.pkl")
    ...
    g_all = pd.concat([g1, g2, g3], axis=0)
    ...
    df_rank_new = pd.DataFrame(df_rank['feature_group'].tolist(), columns=["G1", "G2", "G3"],
                                           index=df_rank.index)

    df_rank_new['feature_group'] = pulsar_optuna_all_case['feature_group']
    df_rank_new['sum'] = df_rank_new[['G1', 'G2', 'G3']].sum(axis=1)
    df_rank_new['f1'] = pulsar_optuna_all_case['result']

    df_rank_new.to_pickle("randomforest/output_randomforest/pulsar_optuna_result_rank_rdf.pkl")
