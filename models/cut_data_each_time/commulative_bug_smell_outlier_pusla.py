...
def cumulative_diff(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ['bug', 'd', 'b', 'cp', 'c', 'ooa', 'u']:
        df.loc[:, f'cumulative_positive_{col.lower()}'] = df[f'diff_{col.lower()}_positive'].diff().cumsum()
        df.loc[:, f'cumulative_negative_{col.lower()}'] = df[f'diff_{col.lower()}_negative'].diff().cumsum()
    return df
...
if __name__ == '__main__':
    ...
    ozone = pd.read_pickle('/Users/bill/origin-source-code-bill/Github/output/ozone_filtered_issue_bug.pkl')

    # Apply the custom clustering function and percentage calculation
    df_calulate_smell_bug = calculate_smell_bug(pulsar)
    ...
    plot_show_shape_outlier_year(data_years, lower, upper, 'pulsar')
    plot_show_shape_outlier_year_and_month(data_years_month, lower, upper, 'pulsar')
