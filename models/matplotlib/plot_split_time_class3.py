...
def polt_3d_html(x_label, y_label, z_label, project_name):
    ...
    fig.show()
    fig.write_html(f"../matplotlib/output/{project_name}_split_{z_label.name}.html", include_plotlyjs=True, full_html=True)
    return fig.show()
...
def polt_3d_subplot(x_label, y_label, z_label, project_name):
    ...
    plt.title(f'{project_name} split {z_label.name} vs time01 and time12')

    plt.savefig(os.path.join(f'../matplotlib/output/{project_name}_split_{z_label.name}_point.png'))
