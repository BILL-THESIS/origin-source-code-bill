...
if __name__ == '__main__':
    # Load the data
    ozone_api = pd.read_parquet('../../models/output/pulsar_prepare_to_train_newversion_9Sep.parquet')
    ...
    percentiles = calculate_percentiles(ozone_api['total_time_hours'])
    ...
    time_classes = divide_time_class_2(ozone_api, time_points)
    df_good, ozone_bad = check_amount_time_class(time_classes)


    pulsar_result = split_data_x_y(df_good, before_2024, after_2024)

    # Convert the list of dictionaries into a DataFrame
    pulsar_result_df = pd.DataFrame(pulsar_result)

    pulsar_result_df['std_counts'] = pulsar_result_df[['time0', 'time1', 'time2']].std(axis=1)
    pulsar_result_df['std_f1'] = pulsar_result_df[['f1_score_class0', 'f1_score_class1', 'f1_score_class2']].std(axis=1)
    pulsar_result_df['std_f1_smote'] = pulsar_result_df[['f1_smote_class0', 'f1_smote_class1', 'f1_smote_class2']].std(axis=1)

    # Save the DataFrame to a parquet file
    with open('../../models/output/pulsar_test_model_svc_timeclass3_28Sep.parquet', 'wb') as f:
