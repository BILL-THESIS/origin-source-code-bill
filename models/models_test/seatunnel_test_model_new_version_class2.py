...
if __name__ == '__main__':
    # Load the data
    seatunnel_api = pd.read_parquet('../../models/output/seatunnel_prepare_to_train_newversion_9Sep.parquet')
    seatunnel_api = percentage_smell(seatunnel_api)

    # Check the data time out of 2024
    before_2024, after_2024 = check_time_df(seatunnel_api)

    # prepare percentile to divide time class for 2 classes
    percentile_list = [np.percentile(seatunnel_api['total_time_hours'], range(1, 101))]
    percentile_df = pd.DataFrame(percentile_list).T

    data_time_class_list, data_time_point_list = process_data(seatunnel_api, percentile_df)

    seatunnel_result = split_data_x_y(data_time_class_list, before_2024, after_2024)

    # Convert the list of dictionaries into a DataFrame
    seatunnel_result_df = pd.DataFrame(seatunnel_result)

    with open('../../models/output/seatunnel_teat_model_somte_newversion_class2_18Sep.parquet', 'wb') as f:
        joblib.dump(seatunnel_result_df, f)
        print("save file Done!")
        print(seatunnel_result_df)  # This will now print the DataFrame containing the concatenated results
