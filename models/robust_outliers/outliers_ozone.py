...
if __name__ == '__main__':
    model_original = pd.read_pickle('../../Github/output/ozone_filtered_final_api_new.pkl')
    ...
    # Save plot
    plt.savefig('../output/ozone_filtered_robust_outlier.png')

    end = df_normal['merge_commit_sha'].drop_duplicates()
    end.to_csv('../output/tracking_api_to_sonar/ozone_filtered_robust_outlier_end_new.txt', header=True, index=False)
    start = df_normal['base.sha'].drop_duplicates()
    start.to_csv('../output/tracking_api_to_sonar/ozone_filtered_robust_outlier_start_new.txt', header=True, index=False)
