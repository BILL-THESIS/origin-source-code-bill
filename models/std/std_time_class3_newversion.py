...
def compare_std(datasets, labels):
    sd_values = {}
    cv_values = {}

    for label, data in zip(labels, datasets):
        std_time = data[['time01_x', 'time12_x']].std().mean()
        mean_time = data[['time01_x', 'time12_x']].mean().mean()

        # Save SD and CV for comparison
        sd_values[label] = std_time
        cv_values[label] = std_time / mean_time * 100

        print(f"{label} - SD: {std_time:.2f}, CV: {cv_values[label]:.2f}%")

    # Perform F-test between datasets
    f_statistic, p_value = stats.f_oneway(*(data[['time01_x', 'time12_x']].std() for data in datasets))
    print(f"F-test statistic: {f_statistic}, P-value: {p_value}")

    return sd_values, cv_values
...
if __name__ == '__main__':
    ...
    # Compare SD and CV
    datasets = [ozone_std, pulsar_std, seatunnel_std]
    labels = ['ozone', 'pulsar', 'seatunnel']
    sd_values, cv_values = compare_std(datasets, labels)
